...
if __name__ == "__main__":
    ...
    try:
        ...
        agent = ReflectionAgent(llm_client=llmClient, max_iterations=5)

        exampleTask = " 编写一个Python函数，找出1到n之间所有的素数 (prime numbers)。"
        finalCode = agent.run(exampleTask)
    except ValueError as e:
        print(f"❌ 初始化LLM客户端时发生错误: {e}")
